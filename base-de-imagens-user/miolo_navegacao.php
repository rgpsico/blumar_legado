...
$pega_tipos="select distinct tp_produto from banco_imagem.bco_img where tp_produto in ('1','10') order by tp_produto";
...
$select_cidade=' <select name="cidade_cod" id="cidade_cod"  size="30" >';
if(strlen($sected) != 0)
{
...
{
	$select_cidade= $select_cidade.'<option value="0" selected>----- Choose a city -----</option>';
}
...
for ($rowtp = 0; $rowtp < pg_numrows($result_tipos); $rowtp++) {
    ...
    if($tp_produto == '1') {  echo'<div id="boxselect1">';   }
	elseif($tp_produto == '10') {   echo'<div id="boxselect10">';  }	// echo$tp_produto.'<br>';

	if($tp_produto == '1') {    echo'HOTELS ';   }
	...
	elseif($tp_produto == '10') {    echo'CITIES AND TOURS '; }
...
else
{
   ...
   echo'<option value="0" selected>----- Choose a city -----</option>';
...
echo'</div>';
	...
	elseif($tp_produto == '11') {  echo'<div id="miolo_prod11"></div> ';  }  ;	
