...
if(isset($_SESSION["lista_download"])){    } else {  $_SESSION["lista_download"] = array(); }
if(isset($_SESSION["lista_images_download"])){    } else {  $_SESSION["lista_images_download"] = array(); }
...
echo'<div id="mapa-eco" ></div><div id="cabeca_bancoimg"><b>  '.strtoupper($umhtl).' PICTURES  </b> <strong>|</strong> <a href="https://www.blumar.com.br/client_area/rates/new_pop_hotel.php?cod_hotel='.$mneu_for.'&lang=2&hotel='.strtoupper($umhtl).'" target="_new">Check hotel page >></a></div>';
...
for ($rowcid = 0; $rowcid < pg_numrows($result_cidadestp); $rowcid++)
{
	...
	$fachada = pg_result($result_cidadestp, $rowcid, 'fachada');
	 ...
	 echo'<div id="tumb_bancoimg"><div class="legenda">';

		if(strlen($legenda) != 0)
		{
			echo' <b>'.ucfirst($legenda).' | &copy; '.ucfirst($autor).'</b>';
		}

		echo'<div class="select_legenda"><a href="#shopping_cart" onclick="showDiv()" class="add_download"><input type="hidden" class="add_downloadvalue" value="'.$pk_bco_img.'"> add + </a></div></div>';
